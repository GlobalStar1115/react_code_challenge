...
const CampaignKeywordsCleanerNonProfitableTable = ({ campaignType, currentAdGroupId, selectedKeyword }) => {
  ...
  const fields = [
    ...
    { value: 'sales', label: 'Revenue', sort: true, dataType: 'number', },
  ]
  ...
  useEffect(() => {
    ...
    if (!dashboardKeywordCleanerRelatedTerms.unprofitableTerms || dashboardKeywordCleanerRelatedTerms.unprofitableTerms.length === 0) {
      setTerms([])
      return
    }
    let tmpTerms = [...dashboardKeywordCleanerRelatedTerms.unprofitableTerms]
    ...
    tmpTerms = tmpTerms.map(term => ({
      ...
      acos: formatValue(term.acos, 'number'),
      cost: formatValue(term.cost, 'number'),
      sales: formatValue(term.sales, 'number'),
  ...
  return (
    <div className="table-container">
      <div className="table-title">
        <h5>Unprofitable search terms (based on acos) </h5>
      </div>
...
export default CampaignKeywordsCleanerNonProfitableTable