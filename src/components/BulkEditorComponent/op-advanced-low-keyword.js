...
const OptimizationAdvancedLowKeywordComponent = (props) => {
  const store = useStore()

  const { updateBulkData, updateBid, changeToNewBid } = props

  const { campaign } = store.getState()
  const { optAdvancedLowKeywordsBulk, isOptAdvancedLowKeywordsBulkLoading } =
    campaign
  ...
  useEffect(() => {
    if (
      ...
      optAdvancedLowKeywordsBulk.length === 0
    ) {
      return
    }
    const newKeywords = optAdvancedLowKeywordsBulk.map((keyword) => {
  ...
  }, [optAdvancedLowKeywordsBulk, selectedKeywords])
  ...
  const handleAdjustBid = () => {
    ...
    updateBid('FindLow', selectedAdjustBidOption, bidValue, selectedKeywords)
  ...
  const handleChangeToNewBid = () => {
    changeToNewBid('FindLow', selectedKeywords)
  ...
  return (
    <div
      className={
        isOptAdvancedLowKeywordsBulkLoading
          ? 'optimization-advanced-keyword loading'
          : 'optimization-advanced-keyword'
      }
    >
      {isOptAdvancedLowKeywordsBulkLoading && <LoaderComponent />}
...
export default OptimizationAdvancedLowKeywordComponent
