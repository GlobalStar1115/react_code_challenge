...
const OptimizationAdvancedMostKeywordComponent = (props) => {
  ...
  const {
    ...
    isOptAdvancedMostKeywordsBulkLoading,
    isUpdateOptAdvancedMostKeywordListStateBulk,
  } = campaign
  ...
  useEffect(() => {
    if (
      !optAdvancedMostKeywordsBulk ||
      optAdvancedMostKeywordsBulk.length === 0
    ) {
      return
    }
    const newKeywords = optAdvancedMostKeywordsBulk.map((keyword) => {
  ...
  }, [optAdvancedMostKeywordsBulk, selectedKeywords])
  ...
  const handleChangeKeywordsState = (type) => {
    updateBulkData('advanced-most-keyword', type, selectedKeywords)
  ...
  const handleAdjustBid = () => {
    ...
    updateBid('FindMost', selectedAdjustBidOption, bidValue, selectedKeywords)
  ...
  const handleChangeToNewBid = () => {
    changeToNewBid('FindMost', selectedKeywords)
  ...
  return (
    ...
    >
      {(isUpdateOptAdvancedMostKeywordListStateBulk ||
        isOptAdvancedMostKeywordsBulkLoading) && <LoaderComponent />}
...
export default OptimizationAdvancedMostKeywordComponent
