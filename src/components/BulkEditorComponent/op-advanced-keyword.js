...
const OptimizationAdvancedKeywordComponent = (props) => {
  ...
  const {
    ...
    isOptAdvancedKeywordsBulkLoading,
  } = campaign
  ...
  useEffect(() => {
    if (!optAdvancedKeywordsBulk || optAdvancedKeywordsBulk.length === 0) {
      return
    }
    const newKeywords = optAdvancedKeywordsBulk.map(keyword => {
      const units = parseInt(keyword.units, 10)
      const maxcpc = keyword.clickorderratio && units ? ((keyword.revenue / units) * (keyword.average_acos / 100)) / keyword.clickorderratio : 0
      return (
        {
          ...keyword,
          bid: keyword.bid ? formatValue(keyword.bid, 'currency', 2) : '0',
          revenue: keyword.revenue ? formatValue(keyword.revenue, "currency", 2) : '$0',
  ...
  }, [optAdvancedKeywordsBulk, selectedKeywords])
  ...
  const handleChangeKeywordsState = ( type ) => {
    updateBulkData('advanced-keyword', type, selectedKeywords)

    const selectedIds = selectedKeywords.map(keyword => keyword.id)

    setKeywordData(keywordData.map(keyword => {
      return selectedIds.includes(keyword.id) ?
        {
          ...keyword,
          state: type,
        } : keyword
  ...
  const handleAdjustBid = () => {
    ...
    setKeywordData(keywordData.map(keyword => {
      if (selectedIds.includes(keyword.id) && keyword.keywordtext !== '(_targeting_auto_)') {
        newBid = formatValue(bidValue, 'number', 2)
        if (selectedAdjustBidOption.value === 'raiseBid') {
          newBid = parseMoneyAsFloat(keyword.bid, "$") * (1 + parseFloat(bidValue) / 100)
  ...
  const handleChangeToNewBid = () => {
    ...
    const selectedIds = selectedKeywords.map(keyword => keyword.id)
    setKeywordData(keywordData.map(keyword =>
      selectedIds.includes(keyword.id) && keyword.keywordtext !== '(_targeting_auto_)' && keyword.newbid !== 'n/a' && keyword.newbid >= 0.02 ?
        {
          ...keyword,
          bid: keyword.newbid,
        } : keyword
  ...
  return (
    <div className={ isOptAdvancedKeywordsBulkLoading ? "optimization-advanced-keyword loading" : "optimization-advanced-keyword" }>
      { isOptAdvancedKeywordsBulkLoading && <LoaderComponent />}
      {
        selectedKeywords && !isShowAdjustBid && selectedKeywords.length > 0 &&
          <div className="sku-action">
            ...
            <button
              ...
              onClick = {() => { handleChangeKeywordsState(e, 'paused') }}
            >
              Pause
            </button>
            <button
              ...
              onClick = {() => { handleChangeKeywordsState(e, 'archived') }}
            >
              Archive
            </button>
            <button
...
export default OptimizationAdvancedKeywordComponent
