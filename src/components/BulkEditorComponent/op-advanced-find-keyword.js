...
const OptimizationAdvancedFindKeywordComponent = (props) => {
  const store = useStore()

  const { updateBulkData, updateBid, changeToNewBid } = props

  const { campaign } = store.getState()
  const { optAdvancedFindKeywordsBulk, isOptAdvancedFindKeywordsBulkLoading } =
    campaign
  ...
  useEffect(() => {
    if (
      ...
      optAdvancedFindKeywordsBulk.length === 0
    ) {
      return
    }
    const newKeywords = optAdvancedFindKeywordsBulk.map((keyword) => {
  ...
  }, [optAdvancedFindKeywordsBulk, selectedKeywords])
  ...
  const handleChangeToNewBid = () => {
    changeToNewBid('FindKeywords', selectedKeywords)
  ...
  return (
    <div
      className={
        isOptAdvancedFindKeywordsBulkLoading
          ? 'optimization-advanced-keyword loading'
          : 'optimization-advanced-keyword'
      }
    >
      {isOptAdvancedFindKeywordsBulkLoading && <LoaderComponent />}
...
export default OptimizationAdvancedFindKeywordComponent
