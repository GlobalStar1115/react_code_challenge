...
const OptimizationAdvancedZeroKeywordComponent = (props) => {
  const store = useStore()

  const { updateBulkData, updateBid, changeToNewBid } = props

  const { campaign } = store.getState()
  const { optAdvancedZeroKeywordsBulk, isOptAdvancedZeroKeywordsBulkLoading } =
    campaign
  ...
  useEffect(() => {
    if (
      ...
      optAdvancedZeroKeywordsBulk.length === 0
    ) {
      return
    }
    const newKeywords = optAdvancedZeroKeywordsBulk.map((keyword) => {
  ...
  }, [optAdvancedZeroKeywordsBulk, selectedKeywords])
  ...
  const handleChangeKeywordsState = (type) => {
    updateBulkData('advanced-zero-keyword', type, selectedKeywords)
  ...
  const handleAdjustBid = () => {
    ...
    updateBid('FindZero', selectedAdjustBidOption, bidValue, selectedKeywords)
  ...
  const handleChangeToNewBid = () => {
    changeToNewBid('FindZero', selectedKeywords)
    setSelectedKeywords(
      selectedKeywords.map((selectedKeyword) =>
  ...
  return (
    <div
      className={
        isOptAdvancedZeroKeywordsBulkLoading
          ? 'optimization-advanced-keyword loading'
          : 'optimization-advanced-keyword'
      }
    >
      {isOptAdvancedZeroKeywordsBulkLoading && <LoaderComponent />}
...
export default OptimizationAdvancedZeroKeywordComponent
